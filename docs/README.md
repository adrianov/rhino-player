...
| [~60 fps motion (VapourSynth)](features/26-sixty-fps-motion.md) | **Done** (menu **Preferences** → **Smooth video (60 FPS)**; `video_smooth_60`, `video_vs_path`; bundled `data/vs/*.vpy`) |
...
| [Window: size, fullscreen, UI auto-hide, inhibit idle](features/17-window-behavior.md) | In progress (fullscreen, `GtkWindowHandle`, chrome autohide, cursor hide, fit-on-open, inhibit; **not:** post-resize aspect lock — see file) |

...
- Load target: prefer main mpv `path` when it is a local stream file (e.g. Blu-ray `.m2ts` under `STREAM/`) or `bd://` / `bluray://`; else `shell_media_path` + `resolve_open_media_path`. **Separate** libmpv instance (`MpvPreviewGl`); never seeks or loads on the main player. Concurrent software decode can still starve Smooth / VapourSynth on the main path — preview downscales via **`PREVIEW_DECODE_MAX_W`** (`preview_media_load.rs`), uses **`vd-lavc-threads=1`**, longer debounce when main **`vf`** has VapourSynth, and scrub seeks use the frame pump (not synchronous decode per debounce tick). Preview always uses software decode (`hwdec=no`); DVD `.vob` uses `hr-seek=yes` on the preview player only. Preview always `vf clr` before `loadfile` and `keep-open=always` so EOF does not unload. Hover/seek times use the minimum of seek-bar upper, main `duration`, and preview `duration`, capped ~1–4 s before the end. Debounced seeks use `absolute+exact` on optical / `absolute+keyframes` otherwise; a frame pump waits for `vo-configured` then renders (scrub reuses the same pump). Leaving the seek bar hides the overlay only — cached load target and owner entity stay until main media change or preview `GLArea` unrealise (macOS theater may unrealise on hide). Reopen after hide skips the 120 ms debounce, repaints the last warm frame immediately (`queue_render`), then seeks. `need_load` also checks `vo-configured` so a fresh GL context reloads after unrealise. On main media change, preview cache and owner entity reset without touching the preview GL context (avoids stealing the main embed context on macOS). Debug: `[rhino] preview:` lifecycle lines always on plain `cargo run`; `RHINO_PREVIEW_DEBUG=1` adds per-tick pump detail; reset sites log `reset from <caller>` (`preview_debug.rs`).
- UI: preview is a **`GtkFrame`** overlay on **`outer_ovl`** (not **`GtkPopover`**) — no separate compositor surface. **macOS theater:** `seek_bar_preview/macos_compositing.rs` — widget-level opaque CSS (gray frame `#2d2d2d`, black preview **`GLArea`**), **`raise_overlay_child` once per fullscreen session** (re-adding unparents the frame and tears down **`GLArea`**), hide uses **opacity 0** (frame stays realised; no compositor refresh on hide), **`on_overlay_surface_opened`** on first theater open only; see [`references-gtk4-macos-header-menus.md`](../references-gtk4-macos-header-menus.md) (**Theater overlay compositing**).

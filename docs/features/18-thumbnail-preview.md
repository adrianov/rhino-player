...
- **Debounce** **70** ms. The debounce `glib::SourceId` must be taken when the timeout **runs** (GLib already removes the source on `Break`); otherwise a later cancel would call `remove` on a stale id and abort.

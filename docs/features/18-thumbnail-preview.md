...
- UI: preview is a **`GtkFrame`** overlay on **`outer_ovl`** (not **`GtkPopover`**) — no separate compositor surface. **macOS:** opaque CSS on the frame at connect (`seek_bar_preview/macos_compositing.rs`); show/hide is **`set_visible` only** (with **`set_opacity(1)`** on reopen so a leftover theater opacity-0 cannot hide the frame); lift uses **`bottom_shell`** height. Theater reopen calls **`macos_shell_compositing::overlay_opened`**, every hide calls **`overlay_closed`**. See [`references-gtk4-macos-header-menus.md`](../references-gtk4-macos-header-menus.md) (**Theater overlay compositing**).

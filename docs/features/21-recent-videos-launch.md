...
Clicking a history card loads that file and unpauses, even if watch-later had stored a paused session. The first history card may be warm-preloaded paused behind the grid on launch; hovering any other card while the grid stays visible may warm-preload that file in the background after the hover settle time. Activating a preloaded card (click or Space) hides the grid and reveals playback after a short reveal delay. Returning to the grid keeps the current file paused for warm reuse when the continue strip stays visible (including empty history while using this launch pattern). Playback stops when browsing back hides the strip (no boot-file launch paths).
...
Feature: Recent videos grid on empty launch
  ...
  Scenario: Hover warm-preloads a continue card in the background
    ...
    When the pointer rests on that card for the hover settle time
  ...
  Scenario: Scrolling past cards does not warm-preload each one
    Given the recent grid is visible and the playback engine is ready
    When the pointer moves across several continue cards faster than the hover settle time
    Then those briefly hovered files do not each start a background warm load
  ...
  Scenario: Uniform-color still is retried nearby
    Given background thumbnail capture at the continue position yields a mostly uniform-color frame
    And a nearby time in the same file shows more picture detail
    When background backfill finishes
    Then the card stores and shows the more detailed nearby frame
    And the uniform-color frame is not kept as the card thumbnail
```
...
- Card UI: each card shares one **16:9** footprint sized as if the strip were full (Open tile + five history slots) so a short list or search hit does not inflate tiles; width is clamped between a minimum and maximum (`card_dims::CARD_*` / `sync_card_sizes`). Thumbnail display uses cover style inside an aspect frame that **obeys the 16:9 ratio, not the still’s natural size**, so a portrait WebP cannot raise the row height and stretch siblings. Cards use start valign in the strip. Title and progress sit in a soft bottom gradient overlay; the percentage is a small translucent pill; **Move to Trash** sits left of **Remove** on hover (shared `rp-recent-action` chrome in `theme/continue_grid.css`). Hover warm preload uses `warm_hover_hooks` / `schedule_warm_hover_preload` (`WARM_HOVER_DEBOUNCE` in `warm_preload_idle.rs`) so scroll-through enters do not each `loadfile`; leave cancels the pending timer. While the continue grid is visible and warm hover is active, `MpvBundle::skip_media_persist` blocks SQLite `media` writes (resume, decode size, smooth ME budget); close / back-from-playback / quit after real playback use `save_playback_state_for_close`. The card title uses `human_media_title` on the basename (Transmission-style cleanup; window title uses the same helper). Incomplete Direct Connect names (`name.ext.<id>.dctmp`) are peeled in `download_temp.rs` before the usual tag cleanup. The leading **Open Video** tile uses the same footprint and `rp-recent-card` chrome plus dashed border styling in `theme/continue_grid.css`; it activates `app.open` (same flow as the **Open Video** menu entry).

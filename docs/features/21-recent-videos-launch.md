...
- Thumbnails: `vo=image` libmpv with high-resolution seeking off; scale to ~960 px wide with `force_original_aspect_ratio=decrease`; JPEG quality ~82; video-only player with no audio / subtitles / external autoload / scripts / resume; loop-filter skipping only. DVD titles: `still_target_from_global` loads the resume chapter `.vob` (same timeline as preview/resume); capture pauses during load, seeks with the same cap and `absolute+exact` mode as seek-bar preview, clears any t=0 `vo=image` spill, then grabs the frame via `screenshot-to-file` (fallback `frames=1`). Cache key includes `thumb_load_path` so a thumb from the wrong VOB is not reused.

...
- Thumbnails: `vo=image` libmpv with high-resolution seeking off; scale to ~960 px wide with `force_original_aspect_ratio=decrease`; JPEG quality ~82; video-only player with no audio / subtitles / external autoload / scripts / resume; loop-filter skipping only. DVD titles: `grid_thumb_target` loads the resume chapter `.vob` at a **local** seek (same mapping as playback), not the first chapter with whole-title seconds.

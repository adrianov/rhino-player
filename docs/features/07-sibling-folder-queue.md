...
**Current code:** `sibling_advance::next_after_eof` (`src/sibling_advance.rs`); `maybe_advance_sibling_on_eof` in `src/app/load.rs` and the 200ms transport tick in `src/app/realize.rs`.

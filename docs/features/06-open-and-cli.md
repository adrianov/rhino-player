...
## Notes
- Open failures (empty/hollow files, demux errors, missing paths) surface a continue-grid notice toast (`src/media_open_fail.rs`, `NoticeToast`) overlaid under the strip like the undo snackbar ([21](21-recent-videos-launch.md)) and return to browse when playback was entered. Zero-filled torrent preallocation is detected before `loadfile`.

...
**Current code:** `adw::Application` with ID `ch.rhino.RhinoPlayer`, `adw::ApplicationWindow`, `adw::ToolbarView` (header + `GLArea` + bottom seek bar), `adw::HeaderBar` with app menu, `app.open` (Ctrl+O), `app.about` (F1), `app.quit` (q, Ctrl+Q), and `gtk::AboutDialog`. `adw::init()` and `adw::StyleManager` force dark.

...
- Selected look at non-1.0×: `playback_speed::stamp_header` toggles the **`rp-speed-on`** class on the speed `MenuButton` (same subtle highlight as `rp-smooth-on` / `rp-blackout-on`, see `theme/shell.css`); applied on row pick, digit shortcuts, transport sync, and post-load `sync_list`.

...
## Notes
- Detection: `video_ext::is_dvd_vob_path`; chapter list = same-title `.vob` files in one `VIDEO_TS/` (`VTS_02_1` groups with `VTS_02_2`, not `VTS_01_*` or `VTS_03_*`), via `dvd_entity::video_ts_for_vob` + `list_title_vobs`, natural sort. The unified timeline queues **every on-disk `.vob`** in that list — no IFO/PTT/chapter metadata. Mid-title EOF uses `advance_title_chapter_eof`. Bottom-bar **Previous** / **Next** and title-end sibling advance use sibling **disc directories** under the same parent ([07](07-sibling-folder-queue.md)), opening `dvd_first_playable_vob` on the neighbour disc. Folder/disc open: `video_ext::dvd_first_playable_vob` — `pick_main_dvd_vob` (most chapters; ties → lowest `VTS_XX`, skip `VTS_01` when `VTS_02+` exist) then SQLite resume on that **entity** key only (not other titles on the disc). Title EOF uses global bar position; mid-title chapter EOF loads the next `.vob` in the same title via `advance_title_chapter_eof`.
- `resolve_global` uses per-`.vob` duration windows (not start-only), so scrubbing maps to the correct file when SQLite has per-file lengths.
- Seek preview: `preview_hover_duration` must not cap the bar to the open chapter’s mpv `duration` while unified mode is active.
- **Persistence:** multi-part DVD titles use one [playback entity](31-playback-entity.md) row (disc folder containing `VIDEO_TS/`); `time_pos_sec` and `duration_sec` are **whole-title** seconds (sum of per-`.vob` lengths). Per-chapter `.vob` rows keep measured `duration_sec` (resume cleared on entity write). Resume on load maps global time → `.vob` file + local offset (`playback_entity` → `dvd_entity::resume_chapter_and_local`).
- Segment lengths for the bar: per-`.vob` **`duration_sec`** rows in SQLite (from mpv on each open file via `refresh_dvd_bar` / `set_duration`), in-session `grow_chapter_dur` / `apply_live_chapter_dur`, or byte-size fill **only when mpv or another segment provides an anchor** (no fixed bytes/sec guess). Total = sum of queued `.vob` durations. Bar cache keeps the prior timeline when a `FileLoaded` rebuild would inflate total before mpv `duration` is ready.
- **IFO files on disk** (`dvd_ifo_parse`): **main-title pick** when opening a disc folder; **PTT chapter marks** for seek-bar / preview labels only (`IfoChapterMarks` scaled onto the VOB timeline — not used for `resolve_global` or cross-`.vob` load). Audio/sub stream attrs from `VTS_xx_0.IFO`.
...
- Seek bar preview ([18](18-thumbnail-preview.md)): scrub target file comes from VOB `resolve_global`; hover labels use `DvdBarState::chapter_preview_labels` (IFO **Chapter N** marks scaled to the VOB bar — empty when the title has one PTT chapter or no IFO).

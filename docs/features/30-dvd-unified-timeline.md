...
- Segment lengths for the bar: in-session `DvdBarState` cache (`merge_prior_durs`), mpv live `duration` on the open chapter, then **in-process libmpv** (`dvd_vob_mpv_probe::probe_vob_duration` — `libmpv2` crate, `vo=null`, no subprocess / PATH tools) for any queued `.vob` still missing a length (per-path cache). Total = sum of per-file durations. Bar cache keeps the prior timeline when a `FileLoaded` rebuild would inflate total before mpv `duration` is ready.
- **IFO files on disk** (`dvd_ifo_parse`): **main-title pick** when opening a disc folder; **PTT chapter marks** for seek-bar / preview labels only — one `VTS_xx_0.IFO` per title set in the feature queue (`chapter_labels_for_timeline` scales each block onto measured `.vob` totals; not used for segment lengths, `resolve_global`, or cross-`.vob` load). Audio/sub stream attrs from `VTS_xx_0.IFO`.

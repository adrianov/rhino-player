...
- Segment lengths for the bar: in-session `DvdBarState` cache (`merge_prior_durs`), mpv live `duration` on the open chapter, then **`ffprobe`** (`dvd_vob_ffprobe::probe_vob_duration`) for any queued `.vob` still missing a length (in-process cache per canonical path). Total = sum of per-file durations. Bar cache keeps the prior timeline when a `FileLoaded` rebuild would inflate total before mpv `duration` is ready.

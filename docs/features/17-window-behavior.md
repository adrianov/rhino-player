...
Feature: Window, fullscreen, and presentation
  ...
  Scenario: Baked-in strips do not count toward the fit and snap aspect
    Given a media title whose picture has black strips baked into its frame
    And the strips are known from a finished strip detection or a fresh cached detection result
    When the window is fitted on open or snapped after a manual resize
    Then the aspect used for the target size is the picture content without the strips
    And unknown or still-running detection leaves the fit and snap aspect unchanged
  ...
  Scenario: Leave fullscreen with toolbars showing
    Given the window is fullscreen
    And the header and bottom toolbars are visible
    When the user exits fullscreen
    Then the window leaves fullscreen
    And the header and bottom toolbars are visible
    And the prominent window-management controls grouped with that top toolbar are available
  ...
  Scenario: Chrome autohide while playing
    ...
    And the pointer hides with them
  ...
  Scenario: Pointer stays visible on another display
    ...
    And blackout-other-displays is off
  ...
  Scenario: Enable blackout while playing
    ...
    And the pointer is hidden on those blacked-out displays
...
- Fit-on-open: `chrome_window_video_fit.rs` + `chrome_shell_layout` / `chrome_shell_layout_macos_resync` — landscape fit; macOS **`macos_sync_shell_and_nudge`** (one sync + **`nudge_gdk_compositing_width`**) after GTK size match and on continue-strip hide. Linux **`schedule_shell_layout_sync`**: immediate + one 150 ms settle. Bar reveal invalidate policy: **`apply_chrome`** / **`apply_chrome_force_layers`** (`chrome_apply.rs`); FS/focus use the force path via **`touch_chrome_gl_factory`** in `shell_fullscreen.rs`. Smooth `vf` does not refresh chrome. **`RHINO_SHELL_DEBUG=1`**: **`bottom_h`**, **`shell=…x…`**, **`gdk width nudge`**.
- ToolbarView extends to top and bottom edges so the GLArea fills the available area and chrome overlays the video. Client-side decorations: baseline for `shows-start-title-buttons` / `shows-end-title-buttons` is sampled after map (idle) while chrome first shows—not after a hide—or `apply_chrome` would capture `(false,false)` and restore would leave traffic lights off; invalid pairs are ignored in favor of a short `(true,true)` fallback until GTK reports a decorated side. **macOS stoplights** (`src/macos_traffic_vertical.rs`): Y from fixed compact top-bar height (`TOP_BAR_H`); X shifted once and cached; sync skipped while hidden; FS exit uses `flatten_traffic_lights`. Spurious `fullscreened` re-enter while **`macos_fs_exit`** is armed is ignored (must not cancel leave-restore / stick **`exit_armed`**); **`heal_stuck_exit`** clears a latch once AppKit is windowed; leave-restore re-applies chrome on idle plus one settle retry so lights un-flatten after titlebar rebuild.
- **Fit-on-open:** `chrome_window_video_fit.rs` — landscape **960×540-class** fit only when the window is still the default size or **smaller** than that target (grow-only). The target ratio strips known baked-in black strips first (`strip_known_bars` in `chrome_fullscreen_and_fit.rs`); otherwise keeps size; optional aspect nudge via `snap_size_after_user_resize`. Same-media Smooth `loadfile replace` calls **`suppress_window_fit_for_load`** with that load’s **`warm_file_gen`** (drops a pending debounce; fit schedules for that gen are skipped until a newer load). ME-budget / Smooth `vf` rebuilds do not schedule fit.
- **Post-resize aspect snap:** `aspect_resize_snap.rs` — coded `width`×`height` with known baked-in strips removed (`video_snap_aspect_dims` → `strip_known_bars`) in `WinAspectCell`; the strip source is `video_fill::known_bar_crop` (live `BarProbe` verdict, else a fresh `media.bar_crop` row), mapped with `black_bars::scale_rect_between` when the target pair is not decode space. Snap when width **or** height is within **60%** of aspect-correct; compute one-axis deltas **+W**, **−W**, **+H**, **−H** to match aspect (formulas `W′=round(H×vw/vh)`, `H′=round(W×vh/vw)`); apply the **smallest** delta if **|Δ|/side ≤ 50%**. Debounce 200 ms → `apply_window_outer_size`. macOS **`nudge_gdk_compositing_width`** (±1 px after fit / shell sync) calls **`note_programmatic_win_resize`** so that churn does not count as a manual resize.
...
- **Startup shell:** Continue strip uses `recent_view::ensure_apply_strip` (same query-aware paint as search / Feeling Lucky: SQLite durations + cached WebP thumbs) **before** `present`. libmpv init is queued from `GLArea` realize on the next idle; transport / seek-preview / input wiring runs on the next idle after that (`deferred_after_present.rs`; seek preview only when the preference is on). Warm preload of the first continue file runs on the next idle after transport observers are installed (`run_continue_warm_preload`); card hover uses `warm_hover_hooks` → `transport_sync_warm_browse` (seek bar from stored snap; no hover `loadfile`); startup warm preload keeps a single-flight gate. `recent_visible` is seeded from the continue-strip intent (`want_recent`), not `Widget::is_visible()` (false until the window is mapped). Browse mode is simply the strip's own visibility (`Widget::is_visible()`); playback hides it. macOS release builds used to abort with `g_main_dispatch: assertion failed: (source)` about a second after playback started — the trigger was the display-sleep assertion in `idle_inhibit` (see its module docs), so keep that binding IOKit-only. While the strip is shown, Smooth / VapourSynth resync and the resume seek are deferred until reveal/unpause. Resume is applied on deferred `FileLoaded` (and Duration) only (never before the demuxer is ready).
- **macOS header menus:** Windowed — standard [`GtkMenuButton`] + [`GtkPopover`]; gdk-macos opaque CSS on map/show (`macos_header_menu::wire_popover`); `autohide=false` + capture dismiss on **`outer_ovl`**; 300 ms speed pick guard; defer **`invalidate_window_layers`** while a popover popup exists. **Native theater fullscreen** — popovers detached from buttons; same menu **child** reparented into **`outer_ovl`** overlay panel (no gdk popup surface); class **`rp-header-menu-fs`** for enabled chrome; **`on_overlay_surface_opened`** on panel show — full binding in [`references-gtk4-macos-header-menus.md`](../references-gtk4-macos-header-menus.md) (**Theater overlay compositing**). The Linux primary **Main menu** `MenuButton` is **not** packed on macOS (`pack_header_end` skips slot 0; system menubar owns File/View) — a visible empty chevron was a regression from packing a placeholder `MenuButton`.
- **macOS fullscreen:** Native AppKit style mask is authoritative; GDK **`is_fullscreen`** can stick after exit. **`clear_stale_gtk_fullscreen`** when GDK fullscreen but AppKit is not. **Toggle:** **`chrome_macos_toggle`** — does not use **`fs_transition_try_begin`** (380 ms busy blocked rapid Enter after exit); defers while **`inFullscreenTransition`**. **Exit:** arm **`macos_fs_exit`**, hide the toolbars at once (keep them hidden while armed), wait `TRANSITION_SETTLE`, hop to libdispatch, run **`prep_native_fullscreen_exit`** (hide traffic lights, clear `NSToolbar`), then **`toggleFullScreen:`** (`chrome_macos_unfullscreen_defer`). On enter and prep, install once an **`NSThemeFrame` `_syncToolbarPosition`** reentrancy guard — healthy AppKit never re-enters that method; the macOS 26.5.1 exit bug nested it thousands of times via **`setCustomTitlebarHeight:`** / `_updateTitlebarContainerViewFrameIfNecessary` until stack overflow. Verified-dead ends: pre-zeroing titlebar height alone; clearing `.fullScreen` directly (`NSGenericException` on `-[GdkMacosWindow setStyleMask:]`). Chrome restore from **`fullscreened_notify`** + **`macos_schedule_leave_fs_restore_chrome`**. **Enter:** **`enter fullscreen`** log + **`native_toggle_fullscreen_enter`**. **`RHINO_MACOS_FS_DEBUG=1`**. Notify busy clear **120 ms** on macOS (Linux **380 ms**).
- **Multi-monitor blackout (macOS):** `screen_blackout` — one borderless `NSWindow` per non-viewer `NSScreen` at `NSMainMenuWindowLevel + 1`, solid black, shown with `orderFrontRegardless` (does not become key or activate the app). Each cover installs a blank cursor rect (`attach_blank_cursor_content`) so the pointer vanishes there. The covers accept mouse events so that cursor applies, and stay non-key so the viewer remains active. On when the preference is enabled, the viewer is focused, and playback is in session (`playback_session_active`: path loaded via `mpv_media_state`, continue grid hidden, not user-paused). Engine-held pauses keep blackout up — `screen_blackout_tech_hold.rs` tracks them as a liveness-bounded counter: `begin_tech_hold` / `end_tech_hold` (smooth `vf` swap, arrow-seek burst, chapter scrub) must pair, a hold stops reporting active past `HOLD_LIVENESS`, and an `arm_hold_expiry_watch` pass then re-syncs so a leaked hold can no longer pin "user pause clears blackout" for the rest of the session; both edges of every hold trigger a pass because no pause event follows a hold that ends while playback stays paused. Every decision change prints one `[rhino] blackout:` line (enabled / focused / screens / path / paused / hold depth-live-age). User pause (Space / play / MPRIS / Now Playing) clears covers — as do deactivate, browse overlay, and preference off. Scheduling: GLib idle for GTK only; AppKit create / `orderOut` / `orderFrontRegardless` on libdispatch main (no GTK on that hop — avoids `Idle source dispatched without callback`). Duplicate rebuilds coalesce via `cover_pending`.
  **Pause restores the pointer at once:** ordering the covers out does not make AppKit re-evaluate the cursor until the
  pointer moves, and each cover owns a blank cursor rect -- so the `Clear` op now also runs
  `macos_window::show_system_cursor()` (CoreGraphics show is paired with the theater counter and idempotent when idle).
  Covers use `initWithContentRect:…screen:` with a **screen-local** origin `(0,0)` and

...
- In **fullscreen**, any pointer motion (capture phase on the main window) reveals the top and bottom bars; after **3 seconds** without motion they hide again. While fullscreen, `AdwToolbarView` has **extend content to top and bottom edge** so the `GLArea` is allocated the full area and the **header and bottom toolbars are drawn on top of the video** (overlay), not as extra vertical strips that shrink the video. The status line is in a `GtkOverlay` on the `GLArea` so it does not take layout space from the video.

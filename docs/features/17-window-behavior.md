...
- **macOS exit fullscreen:** after **`fullscreen_timing`** settlement, **`dispatch_async_f`** on libdispatch's main queue posts **⌃⌘F** to AppKit (`macos_window::post_fullscreen_toggle_shortcut`); GTK **`unfullscreen`** runs only via **`dispatch_after_f`** (~350 ms) if still fullscreen (`gtk_unfullscreen_guarded`). Nested zero-duration GLib timeouts and immediate GTK **`unfullscreen`** reproduced `_NSExitFullScreenTransitionController` recursion (~74k frames) on macOS 26.x. libdispatch via **`dlsym`** (`/usr/lib/system/libdispatch.dylib`). Widget **`g_object_ref`** / **`g_object_unref`** on the async tail. **Restore size:** GDK often reports `is_maximized` false while native fullscreen is active; the deferred maximize helper must **not** overwrite `fs_restore` once toggle already saved pre-maximize `(w, h)`, or exit fullscreen restores screen-sized dimensions instead of the original window.

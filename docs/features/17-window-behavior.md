...
- **macOS exit fullscreen:** after **`fullscreen_timing`** settlement, **`dispatch_async_f`** on libdispatch's main queue calls **`native_leave_fullscreen`** (`NSWindow::toggleFullScreen:`) and posts **⌃⌘F** as a supplement — **never** GTK **`unfullscreen`** (gdk-macos **`present`** during `_NSExitFullScreenTransitionController` reproduced ~74k-frame recursion on macOS 26.x). **`dispatch_after_f`** (~350 ms) runs **`sync_gtk_fullscreen_from_native`** (`set_fullscreened(false)` when AppKit has left fullscreen). libdispatch via **`dlsym`** (`/usr/lib/system/libdispatch.dylib`). Widget **`g_object_ref`** / **`g_object_unref`** on the async tail. **Restore size:** GDK often reports `is_maximized` false while native fullscreen is active; the deferred maximize helper must **not** overwrite `fs_restore` once toggle already saved pre-maximize `(w, h)`, or exit fullscreen restores screen-sized dimensions instead of the original window.

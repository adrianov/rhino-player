...
- **macOS exit fullscreen:** after GTK timer / idle deferral (`fullscreen_timing`), **`GtkWindow::unfullscreen`** runs from a GLib idle — **`-toggleFullScreen:` was dropped**: on macOS 26.x it drove `_NSExitFullScreenTransitionController` into ~74k-frame recursion (`_syncToolbarPosition` ↔ `_updateTitlebarContainerViewFrameIfNecessary`) and Rust stack overflow. **Restore size:** GDK often reports `is_maximized` false while native fullscreen is active; the deferred maximize helper must **not** overwrite `fs_restore` once toggle already saved pre-maximize `(w, h)`, or exit fullscreen restores screen-sized dimensions instead of the original window.

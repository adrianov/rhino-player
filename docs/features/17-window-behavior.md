...
- **macOS exit fullscreen:** after GTK timers / idle deferral (`fullscreen_timing`), `-toggleFullScreen:` is deferred again with **nested GTK idle callbacks** so AppKit leaves fullscreen **after** the current layout/notification burst. A synchronous `-toggleFullScreen:` reproduces stack overflow on macOS 26.x (~`_NSExitFullScreenTransitionController`, `_syncToolbarPosition` ↔ `_updateTitlebarContainerViewFrameIfNecessary`, `_NSThemeZoomWidgetCell`).

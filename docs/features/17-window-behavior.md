...
- **macOS exit fullscreen:** after **`fullscreen_timing`** settlement, **`GtkWindow::unfullscreen`** runs after a short chain of **`glib::timeout_add_local_once(Duration::ZERO)`** hops so each call runs in a later main-loop iteration (outside the settle timeout stack). A single **`idle_add_local_once`** nested under **`g_main_context_dispatch`** still hit `_NSExitFullScreenTransitionController` recursion (~74k frames: `_syncToolbarPosition` ↔ `_updateTitlebarContainerViewFrameIfNecessary`) and Rust stack overflow on macOS 26.x. **`-toggleFullScreen:` is not used.** **Restore size:** GDK often reports `is_maximized` false while native fullscreen is active; the deferred maximize helper must **not** overwrite `fs_restore` once toggle already saved pre-maximize `(w, h)`, or exit fullscreen restores screen-sized dimensions instead of the original window.

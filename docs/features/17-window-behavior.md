...
- **macOS fullscreen:** Native AppKit style mask is authoritative; GDK **`is_fullscreen`** can stick after exit. **`clear_stale_gtk_fullscreen`** when GDK fullscreen but AppKit is not. **Toggle:** **`chrome_macos_toggle`** — does not use **`fs_transition_try_begin`** (380 ms busy blocked rapid Enter after exit); defers while **`inFullscreenTransition`**. **Exit:** arm **`macos_fs_exit`**, hide the toolbars at once (keep them hidden while armed), wait `TRANSITION_SETTLE`, hop to libdispatch, run **`prep_native_fullscreen_exit`** (hide traffic lights, clear `NSToolbar`, set titlebar height to zero), then **`toggleFullScreen:`** (`chrome_macos_unfullscreen_defer`). That titlebar reset keeps `_NSExitFullScreenTransitionController` **`setCustomTitlebarHeight:`** from looping `_updateTitlebarContainerViewFrameIfNecessary` ↔ `_syncToolbarPosition` — but on macOS 26.5.1 a stack-overflow crash (SIGABRT via Rust's stack-overflow handler, ~74k recursion levels) still occurred *inside* `prepareToExitFullScreenMode` despite pre-flattened titlebar state. Verified-dead ends: clearing `.fullScreen` directly raises `NSGenericException` ("cleared outside of a full screen transition", `-[GdkMacosWindow setStyleMask:]` in trace); exit-related exception guarding cannot help because the overflow happens synchronously inside AppKit's transition start. Open issue: needs either upstream fix / newer macOS, or interception of the titlebar-height write itself. Chrome restore from **`fullscreened_notify`** + **`macos_schedule_leave_fs_restore_chrome`**. **Enter:** **`enter fullscreen`** log + **`native_toggle_fullscreen_enter`**. **`RHINO_MACOS_FS_DEBUG=1`**. Notify busy clear **120 ms** on macOS (Linux **380 ms**).

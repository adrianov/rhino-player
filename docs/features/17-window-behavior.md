...
- **macOS exit fullscreen:** after GTK timer / idle deferral (`fullscreen_timing`), `-toggleFullScreen:` is deferred with **two chained libdispatch main-queue `exec_async`** hops so AppKit leaves fullscreen after the current CoreFoundation / layout burst (nested GTK idle still reproduced stack overflow on macOS 26.x). **Restore size:** GDK often reports `is_maximized` false while native fullscreen is active; the deferred maximize helper must **not** overwrite `fs_restore` once toggle already saved pre-maximize `(w, h)`, or exit fullscreen restores screen-sized dimensions instead of the original window.

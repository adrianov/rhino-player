...
## Description
A main-menu item **Move to Trash** and the **Delete** / **KP_Delete** shortcut move the playing file to the Freedesktop Trash. The action is enabled only when a local regular file is loaded and the continue grid is hidden. After a successful trash, the app clears watch_later / DB resume for that path, removes it from continue history and the media files catalog, drops it from the continue strip, and otherwise behaves like **Close Video** (see [02-application-shell](02-application-shell.md)). A session **Undo** can untrash the file and restore its snapshot.

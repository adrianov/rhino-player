...
| Area | Behaviour |
|------|-----------|
| `history` table | Append / touch on open via `history::record` from `try_load` when `LoadOpts::record`. Ordered by `last_opened`; capped by `MAX_HISTORY` (`src/db/connection_init_and_audio.rs`). |

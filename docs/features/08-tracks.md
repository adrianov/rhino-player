...
- **Smooth motion (feature 26):** a user pick from the Sound popover (not load-time restore) that changes `aid` runs `video_pref::snap_audio_track_av_resync` **before** `aid` and `finish_audio_track_av_resync` **after** — **only** when the buffering `vapoursynth` vf is in the chain and playback is not paused: pause-if-playing, capture **`playback-time`** / **`time-pos`**, switch `aid`, then `absolute+exact` seek to the captured second and unpause; a **~1 s** tail schedules the same debounced transport resync as arrow-key seeks so the FlowFPS graph can re-init without leaving lip-sync drift. Plain `display-resample` keeps audio synced on its own, so the snap/finish pair is skipped there. Load / chapter restore set `aid` *before* the resume seek (and before unpause), so that one seek re-aligns A/V for the reopened decoder — `apply_file_loaded_resume_and_audio` in `dispatch_sync_ui_file_loaded.rs` and `warm_preload_apply_resume_audio` in `warm_preload_idle.rs` both restore tracks then call `apply_pending_resume`. (Seeking first and switching `aid` after drifted lip-sync on continue until a manual seek.)

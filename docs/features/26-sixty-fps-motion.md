...
## Description
A `vapoursynth` mpv filter runs a `.vpy` script that synthesizes ~60 fps frames from the source. The bundled `data/vs/rhino_60_mvtools.vpy` uses MVTools FlowFPS at full CPU affinity with a deeper VapourSynth queue and `LoadPlugin` resolved from `RHINO_MVTOOLS_LIB`. Rhino passes a **persisted pixel-area budget** (`video_smooth_max_area`, default ~1920×1080): when decode width×height exceeds that budget, the script proportionally **downscales for motion estimation** (√(budget ÷ decode area)), runs **FlowFPS** on that ME raster, and **outputs at that raster size** (~60 fps temporal smoothing; geometry may be smaller than the decoded picture, with the player scaling to the window). While Smooth is on with the bundled script, the app samples **this process’s** CPU on the existing 1 Hz transport tick; if utilization stays above **75%** of logical cores for two consecutive ticks, it recomputes the budget from the **currently saved** value as **saved budget × cores × 0.75 ÷ busy-core-equivalent**, persists it, and rebuilds `vf`. The user toggles intent via main menu **Preferences → Smooth Video (60 FPS)**; the filter runs only at mpv `speed` ~1.0×. A custom `.vpy` path may replace the bundled script (CPU budget adaptation applies only to the bundled path).
...
- The vf line is `vapoursynth:file=…:buffered-frames=16:concurrent-frames=auto:user-data=…` when **`user-data`** is supported (**decimal** clamped budget, **`%n%`** quoting; `.vpy` reads Python **`user_data`**). Older **libmpv** rejects that sub-option: Rhino retries without **`user-data`** and derives **`buffered-frames`** in **15…17** from **`video_smooth_max_area`** so the vf string changes when the budget changes (**`RHINO_SMOOTH_MAX_AREA`** still set). Before **`vf add`**, **`RHINO_SMOOTH_MAX_AREA`** matches the SQLite row (same clamp; shell **`mpv`** without Rhino still uses env). The bundled script **stderr** logs **`smooth_cap=`**, **`path=full`** / **`path=scaled`**, **`decode=`**, **`vf_out=`** — **`vf_out`** is the ME / **FlowFPS** output size (no upscale back to decode); **width** and **height** are floored to multiples of **8** (bundled **blksize**) before **Super** — **Crop** right/bottom on **path=full** when that fits decode, otherwise **Bicubic**; **path=scaled** uses **Bicubic** to the aligned size. **`tier=uhd`** / **`tier=hd`** still reflect decode **≥2560×1440** vs smaller (**logging only**). **Bundled-only**: the **1 Hz** transport tick calls **`getrusage(RUSAGE_SELF)`** deltas → core-equivalent load; if **>** **0.75** × logical cores for **2** consecutive ticks, **`smooth_max_area` ← round(`current_saved_budget × cores × 0.75 / busy_core_equiv`)** (clamped ≥ **320×180**), **`save_video`**, **`apply_mpv_video`** (rebuild **`vf`**). Custom **`.vpy`** skips adaptation. Shared MVTools preset: **`blksize=8`**, **`overlap=0`**, **`Super.levels`** automatic (**`0`**). **`chroma=True`** on **Super** and **Analyse**. **`pel=1`**, **`sharp=1`**, **`search=4`** (hex), **`truemotion=True`**, **`global=True`** on **`Analyse`**, **`FlowFPS`** **`mask=2`**, **`blend=True`**, toward **60/1**. Stderr logs **`tier=`**, **`path=`**, **`decode=`**, **`vf_out=`**. Details: [references-mvtools-super-levels](../references-mvtools-super-levels.md).

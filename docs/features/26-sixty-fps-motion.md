...
- Pause (`pause=yes`) alone keeps the `vf` graph loaded so pause/unpause does not rebuild MVTools. Each **main-player** seek (bottom bar, arrow keys, preview commit) runs **`vf clr`** immediately before **`seek`** whenever vapoursynth was still present; after the seek, a **debounced** **`schedule_smooth_60_resync_idle`** (same **~96 ms** coalescing timer as **`FileLoaded`** / **`path`**) runs **`apply_mpv_video` + `reapply_60_if_still_missing`** when **not** paused so scrubbing restores FlowFPS without a second immediate attach racing unpause. While **paused**, the graph stays cleared after that seek until **`Pause(false)`** schedules that same debounced path. **`apply_pending_resume`** also clears vapoursynth before its **`seek`** so resume positioning does not run through a stale graph. Seek tails use **`request_smooth_60_transport_resync`**. Implemented from libmpv `pause` observation plus seek hooks.

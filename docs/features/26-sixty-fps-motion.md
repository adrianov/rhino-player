...
- The vf line is `vapoursynth:file=…:buffered-frames=16:concurrent-frames=auto` (fixed queue depth). From **`min(width,height)`**: **≤1440** logs **`tier=hd`**, **>1440** **`tier=uhd`**. **`blksize`** is the power of two **nearest** to **`width×height / 10000`** by absolute distance (clamped to MVTools’ usual **4…512** range)—so **1920×1080** (~207) picks **256**, **3840×1600** (~614) and **3840×2160** (~829) pick **512**; **`overlap = blksize / 2`** (integer half). Shared **`pel=1`**, **`sharp=1`**, **`search=4`**, **`truemotion=True`**, **`chroma`** **on** (luma + chroma for **Super** and **Analyse**; luma-only MVs have been seen to skew colors), **`FlowFPS`** **`mask=2`**, **`blend=True`**, **`Super`** uses MVTools default **`levels`** (**0** = automatic pyramid depth; explicit small values truncate the hierarchy and can gut blending). Stderr logs **`tier=`** plus parameters.

...
- The vf line is `vapoursynth:file=…:buffered-frames=24:concurrent-frames=auto` (fixed queue depth). **`mv.Super` / `mv.Analyse`** block size, overlap, and chroma are chosen **inside the bundled `.vpy`** from **`video_in`** using **`min(width,height)`** tiers (~1080 / ~1440 / ~2160 short-edge breakpoints) so heavy presets do not stick after switching to a smaller frame size.

...
- **Linux** (`vo=libmpv` + GTK **`GLArea`**): Plain and Smooth-on both prefer **`video-sync=display-resample`**, **`interpolation=no`**, and **`report_swap`** when the atomic gate is on. **First Smooth-on while playing** (empty **`vf`**, strip flag clear): **`smooth_user_enable_playing_reset`** — hold **`pause`**, **`loadfile replace`** at the playhead (no **`stop`**; it can abort the load), then debounced **`apply_mpv_video`** after **`FileLoaded`** / resume, then **`maybe_unpause_after_smooth_reload`**. **Reattach after strip** (Smooth off→on or post-seek; **`smooth_vf_stripped_this_open`**): skip reload → **`smooth_reattach_after_vf_strip`** (**`vf add`** only; no second keyframe seek, no **`loadfile`**, no **`vf_swap_snap` pause**). On **`vf add`** failure, **`add_smooth_60`** reloads once (`smooth_vf_reload_attempted`) before clearing the pref. **`add_smooth_60`** skips attach while resume is pending; debounced resync retries **`apply_pending_resume`** then attaches. **`vf add` once per attach** (no retry loop / `vf set` — see VSScript pin). **First Smooth-on while paused**: live **`add_smooth_60`**. **Graph replace** (cadence rebuild with live **`vapoursynth`**): **`defer_smooth_vf_swap`** (`smooth-swap`). **Smooth off**: **`vf remove vapoursynth`** via **`clear_vf`**, **`absolute+keyframes`** via **`smooth_off_refresh_playhead`**. Do **not** use **`vf clr`** for teardown — mpv cannot **`vf add vapoursynth`** again on the same file after it. **`VF_SWAP_DEFER_IN_FLIGHT`** blocks stacked defers. **`schedule_smooth_60_resync_idle`** waits for **`resume_seek_pending`**. **320 ms** file-loaded hook updates speed env only when the graph already matches prefs. (No Smooth-off **`loadfile`** — sibling-folder EOF false positives.)

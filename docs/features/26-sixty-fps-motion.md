...
Feature: Optional ~60 fps motion via VapourSynth
  ...
  Scenario: Seek while playing keeps temporal smoothing loaded
    Given Smooth Video is enabled for approximately 1.0× playback
  ...
  Scenario: Seek while paused drops smoothing for a fast still frame
    Given Smooth Video is enabled for approximately 1.0× playback
  ...
  Scenario: Resume after paused seek reattaches smoothing when still enabled
    Given Smooth Video is enabled for approximately 1.0× playback
...
- Pause (`pause=yes`) alone keeps the `vf` graph loaded so pause/unpause does not rebuild MVTools. **`sync_smooth_vf_on_pause_transition`** schedules **`schedule_smooth_60_resync_idle`** on unpause **only** when Smooth is on and the graph was stripped (**`smooth_vf_stripped_this_open`**) or absent — plain pause→resume skips resync when **`vf`** is still loaded. **Main-player seeks while paused** call **`unload_smooth_on_pause`** before **`absolute+keyframes`** (`seek_keyframes.rs`) so the still frame appears quickly — except when **`resume_after_seek_idle`** is latched (transient arrow-burst hold that unpauses at the tail counts as playing); **seeks while playing** keep the graph attached (only the disc cadence gate is marked). Playing seeks still use the post-seek **debounced** tail (**`request_smooth_60_transport_resync`**, **~1 s** for arrow bursts) which no-ops via **`vf_smooth_matches_prefs`** when the graph survives. **`apply_pending_resume`** still clears vapoursynth before its **`seek`** (graph is normally absent there — attach is gated until resume completes). DVD global seeks (**`dvd_vob_timeline_transport`**) still strip before exact seeks across chapter files.

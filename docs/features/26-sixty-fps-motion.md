...
- **Bundled vapoursynth vf:** Rhino attaches `vapoursynth:file=…:buffered-frames=N:concurrent-frames=auto`. **N** is **`SMOOTH_VF_BUFFERED_FRAMES`** in **`src/video_pref/smooth_motion_tier.rs`** (fixed depth, not derived from ME budget). Rhino skips **`vf clr`/`vf add`** only when **`vf_smooth_matches_prefs`** succeeds (resolved script, **`smooth_max_area_env_matches`**, and **`smooth_vf_me_budget_applied.rs`**: persisted ME budget px² equals what the bundled graph **last rebuilt with** (**mpv+VapourSynth** may keep one Python instance when **`vf`** text is unchanged, so **`RHINO_SMOOTH_MAX_AREA`** env refresh alone cannot move **`smooth_cap`**). Any **`video_smooth_max_area`** change forces **`vf clr`/`vf add`**. Optional **`user_data`** / **`user-data=`**: shell **mpv** without Rhino may still populate digits. **stderr** includes **`smooth_cap=`**, **`path=`**, **`decode=`**, **`vf_out=`** — **`vf_out`** is the ME / **FlowFPS** raster without upscaling back to decode dimensions; geometry is aligned/cropped per **`blksize`** helpers in **`data/vs/rhino_60_mvtools.vpy`** (**`overlap` &lt; `blksize`** stays an MVTools rule). Decode-area **tier** tags for logging use **`_MV_DECODE_TAG_AREA`** beside the same script. **Bundled-only** CPU overload that shrinks **`video_smooth_max_area`**: **`smooth_budget.rs`**. **MVTools filter arguments** (**`Super`**, **`Analyse`**, **`FlowFPS`**, masking, cadence targets): **only** **`data/vs/rhino_60_mvtools.vpy`**; **`levels`** background: [references-mvtools-super-levels](../references-mvtools-super-levels.md).

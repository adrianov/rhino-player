...
- **Linux** (`vo=libmpv` + GTK **`GLArea`**): Smooth **on**: **`video-sync=display-resample`**, **`interpolation=no`**, **`report_swap`** when the atomic gate is on. **`clear_vf`** does **not** drop **`report_swap`** before **`vf clr`** — **`restore_non_smooth_present_opts`** sets **`video-sync=audio`** first, **then** clears the gate (**`SeqCst`**), so mpv never sits on **`display-resample`** without swaps. Stripping the graph restores **`hwdec=auto`** / **`vd-lavc-dr=auto`**. **`teardown_gl_paint`** does not add an extra **`report_swap`**. Smooth **off**: **`seek`** **`absolute+exact`** after **`vf clr`** (**no** **`loadfile`** — avoids sibling-folder EOF false positives).
- **macOS** (`vo=libmpv` + **`CVDisplayLink`**): Plain playback uses **`video-sync=display-resample`** + **`report_swap`** (same compositor clock as Smooth-on without vapoursynth). **`audio`** sync fought **`CVDisplayLink`** vsync after Smooth **off**. **`clear_vf`** uses **`with_macos_vf_teardown`** when bundled; Smooth-off **`seek`** refresh + **`mpv_render_context_update`** / **`mark_pending`** follow ( **`unload_smooth_on_pause`** skips the bundle bracket).
- The vf line is `vapoursynth:file=…:buffered-frames=16:concurrent-frames=auto` (fixed queue depth). Before **`vf add`**, Rhino sets **`RHINO_SMOOTH_MAX_AREA`** from **`video_smooth_max_area`** (clamped ≥ **320×180**). The bundled script **stderr** logs **`smooth_cap=`**, **`path=full`** / **`path=scaled`**, **`decode=`**, **`vf_out=`** — when **`path=scaled`**, **`vf_out`** is the ME / **FlowFPS** output size (no upscale back to decode). **`tier=uhd`** / **`tier=hd`** still reflect decode **≥2560×1440** vs smaller (**logging only**). **Bundled-only**: the **1 Hz** transport tick calls **`getrusage(RUSAGE_SELF)`** deltas → core-equivalent load; if **>** **0.75** × logical cores for **2** consecutive ticks, **`smooth_max_area` ← round(`2073600 × cores × 0.75 / busy_core_equiv`)** (clamped), **`save_video`**, **`apply_mpv_video`** (rebuild **`vf`**). Custom **`.vpy`** skips adaptation. Shared MVTools preset: **`blksize=128`**, **`overlap=32`**, **`Super.levels`** automatic (**`0`**). **`chroma=True`** on **Super** and **Analyse**. **`pel=1`**, **`sharp=1`**, **`search=4`** (hex), **`truemotion=True`**, **`global=True`** on **`Analyse`**, **`FlowFPS`** **`mask=2`**, **`blend=True`**, toward **60/1**. Stderr logs **`tier=`**, **`path=`**, **`decode=`**, **`vf_out=`**. Details: [references-mvtools-super-levels](../references-mvtools-super-levels.md).

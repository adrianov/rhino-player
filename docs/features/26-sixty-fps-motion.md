...
When overload coincides with a **busy machine where other apps dominate processor capacity** (for example a heavy compile), Smooth **does not** shrink the persisted budget: it **pauses the temporal-smoothing graph for about one minute**, keeps the user’s Smooth preference on, shows a brief busy-system warning on the Smooth control, then rechecks—extending the pause while other apps still dominate, or restoring the graph when the machine is quieter. When overload is driven mainly by the **viewer process itself**, budget lowering remains the response.
...
Feature: Optional ~60 fps motion via VapourSynth
  ...
  Scenario: Busy-system pause ends when other apps quiet down
    Given smooth motion is paused because other apps dominated processor capacity
    When about one minute passes and other apps no longer dominate processor capacity relative to the viewer process
  ...
  Scenario: Busy-system pause extends while other apps stay busy
    Given smooth motion is paused because other apps dominated processor capacity
    When about one minute passes and other apps still dominate processor capacity relative to the viewer process
    Then the temporal-smoothing filter graph stays paused
    And the busy-system warning remains

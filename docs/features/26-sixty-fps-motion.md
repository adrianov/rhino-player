...
- The vf line is `vapoursynth:file=…:buffered-frames=16:concurrent-frames=auto` (fixed queue depth). From **`min(width,height)`**: **≤1440** logs **`tier=hd`**, **>1440** **`tier=uhd`**; **both** tiers share **`blksize=256`**, **`overlap=128`**, **`pel=1`**, **`sharp=1`**, **`search=4`**, **`truemotion=True`**, **`chroma`** **off** (luma-only for **Super** and **Analyse**), **`FlowFPS`** **`mask=2`**, **`blend=True`**, **`Super`** uses MVTools default **`levels`** (**0** = automatic pyramid depth; explicit small values truncate the hierarchy and can gut blending). Stderr logs **`tier=`** plus parameters.

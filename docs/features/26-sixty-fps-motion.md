...
## Description
A `vapoursynth` mpv filter runs a `.vpy` script that synthesizes ~60 fps frames from the source. The bundled `data/vs/rhino_60_mvtools.vpy` uses MVTools FlowFPS at full CPU affinity with a deeper VapourSynth queue and `LoadPlugin` resolved from `RHINO_MVTOOLS_LIB`; **large decodes** (see Notes threshold) run MVTools on a half-sized raster and leave output at that size (no upscale). The user toggles intent via main menu **Preferences → Smooth Video (60 FPS)**; the filter runs only at mpv `speed` ~1.0×. A custom `.vpy` path may replace the bundled script.
...
Feature: Optional ~60 fps motion via VapourSynth
  ...
  Scenario: Very large decode runs temporal smoothing at a reduced output raster
    ...
    Then the bundled script scales the picture down for motion estimation and interpolated output
    And the viewer sees that reduced raster without enlargement back to the full decode dimensions
...
- The vf line is `vapoursynth:file=…:buffered-frames=16:concurrent-frames=auto` (fixed queue depth). Stderr **`tier=uhd`** when **decode** **`width × height ≥ 2560×1440`** pixels; otherwise **`tier=hd`** (**logging** from decode size). **`tier=hd`**: **`mv.Super`** / **`Analyse`** / **`FlowFPS`** on **native decode** (**stderr `path=full`**). **`tier=uhd`**: **`resize.Bicubic`** to **half** width×height for MVTools only (**`path=half`**); output **stays** half (**no** upscale — e.g. 4K → ~1080p). Shared MVTools preset: **`blksize=128`**, **`overlap=32`**, **`Super.levels`** automatic (**`0`** — pyramid depth from frame size; stderr **`Super.levels=auto`**). **`chroma=True`** on **Super** and **Analyse**. **`pel=1`**, **`sharp=1`**, **`search=4`** (hex), **`truemotion=True`**, **`global=True`** on **`Analyse`**, **`FlowFPS`** **`mask=2`**, **`blend=True`**, toward **60/1**. Stderr logs **`tier=`**, **`path=`**, **`decode=`**, **`me=`**. Details: [references-mvtools-super-levels](../references-mvtools-super-levels.md).

...
```gherkin
@status:done @priority:p1 @layer:playback @area:smooth-60
Feature: Optional ~60 fps motion via VapourSynth
  ...
  Scenario: CLI diagnostics reports Smooth prerequisites
    Given the user wants to verify Smooth Video setup without opening the player
    When they launch with the diagnostics switch
    Then a readiness report covers the temporal-smoothing filter, script host, and motion plugin
    And the process exits without opening the main window
  ...
  Scenario: Seek while playing unloads smoothing to avoid a black flash
    Given Smooth Video is enabled for approximately 1.0× playback
    And playback is not paused
    When the viewer changes playback position
    Then the temporal-smoothing filter graph is unloaded for that position change
    And the picture at the new position appears without a brief black flash
    And playback continues without pausing
    And after a short settle the temporal-smoothing filter graph is restored when the preference is still on

  Scenario: Seek while paused unloads smoothing for a quick still frame
    Given Smooth Video is enabled for approximately 1.0× playback
    And playback is paused
    When the viewer changes playback position before resuming
    Then the temporal-smoothing filter graph is unloaded for that position change
    And the viewer sees the new position without waiting for motion estimation

  Scenario: Resume after paused seek restores smoothing when still enabled
    Given Smooth Video is enabled for approximately 1.0× playback
    And playback is paused after a position change that unloaded the temporal-smoothing filter graph
    When the viewer resumes playback
    Then the temporal-smoothing filter graph is restored when the preference is still on
  ...
  Scenario: Sustained elevated smooth-playback strain rate lowers the saved pixel budget
    ...
    And other apps on the machine are not dominating processor capacity relative to the viewer process
    When presentation strain at the output exceeds about twenty percent of the pacing reference for about three successive transport-heartbeat evaluations across a short trailing window
  ...
  Scenario: Overload from other apps pauses smoothing instead of shrinking the budget
    Given the smooth-motion preference is on with the bundled script
    And playback is progressing without the decoder core stalled idle
    And other apps on the machine dominate processor capacity relative to the viewer process
    When presentation strain at the output exceeds about twenty percent of the pacing reference for about three successive transport-heartbeat evaluations across a short trailing window
    Then the temporal-smoothing filter graph is paused
    And the smooth-motion preference stays on
    And the persisted motion-estimation pixel-area budget is unchanged
    And the smooth control shows a busy-system warning with an explanatory tooltip

  Scenario: Busy-system pause ends after about one minute
    Given smooth motion is paused because other apps dominated processor capacity
    When about one minute has passed
    Then the temporal-smoothing filter graph is restored while the smooth-motion preference stays on
    And the busy-system warning clears

  Scenario: Busy-system pause can end while the playback window is in the background
    Given smooth motion is paused because other apps dominated processor capacity
    When about one minute has passed while the playback window is inactive, unfocused, or minimized
    Then the temporal-smoothing filter graph is restored while the smooth-motion preference stays on
    And the busy-system warning clears

  Scenario: Busy-system pause may return if overload comes back
    Given smooth motion was paused for other apps and has been restored after about one minute
    And other apps still dominate processor capacity relative to the viewer process
    When presentation strain again exceeds about twenty percent of the pacing reference for about three successive transport-heartbeat evaluations across a short trailing window
    Then the temporal-smoothing filter graph is paused again
    And the busy-system warning returns
  ...
  Scenario: Disable Smooth near the same playhead
  ...
  Scenario: Re-enable Smooth without seeking stays aligned
  ...
  Scenario: Enable Smooth while playing preserves the playhead
    Given the smooth-motion preference is off
    And opened media is playing at approximately normal playback speed
    When the viewer turns smooth motion on without seeking
    Then audio and video stay aligned near the same position
    And playback does not start from the beginning before that position returns

  Scenario: Smooth toolbar shows the playing frame rate
    Given opened media is open
    When the Smooth control readout updates
    Then it shows the measured playing frame rate
    And selecting Smooth alone does not force a target rate on the readout

  Scenario: Smooth toolbar follows the smoothed output rate
    Given opened media is playing at approximately normal playback speed
    And smooth motion is producing about sixty frames per second
    When the Smooth control readout updates
    Then the readout is about sixty
  ...
  Scenario: Packaged app prefers a bundled motion plugin when present
    Given the application package includes the motion-interpolation plugin
    And the smooth-motion preference is on at approximately normal playback speed
    When the viewer opens media
    Then temporal smoothing attaches using the packaged plugin
    And a separate system plugin install is not required for that attachment
```
...
- **Busy-system pause** (bundled Smooth only): on overload, when **1‑minute load average ÷ logical CPUs** is high **and** this process’s recent CPU share is **not** dominant, **`smooth_load_hold.rs`** pauses **`vf`** for **`LOAD_HOLD_SECS`** without changing **`smooth_me_budget_px2`** / **`video_smooth_max_area`**. Thresholds: **`EXTERNAL_LOADAVG_PER_CPU`**, **`SELF_CPU_NOT_DOMINANT_FRAC`**. While paused, **`smooth_wants_vapoursynth_vf`** is false so attach/rebuild stays off; preference **`video_smooth_60`** stays on. Toolbar: yellow **`dialog-warning-symbolic`** (`.rp-smooth-load-warn` in **`theme/shell.css`**) + tooltip. After **`LOAD_HOLD_SECS`**, **`apply_mpv_video`** restores Smooth even if the shell is unfocused (strain overload / recovery still skip inactive windows); sustained external overload can pause again via the normal strain ladder when focused. Player-dominated overload still uses **`budget_after_decoder_overload`**.
- **Debug:** **`RHINO_SMOOTH_DROP_STATS=1`** — stderr **≈every 5 s** while bundled Smooth **`vf`** is active (**`smooth_budget`**): **`[rhino] smooth: stats`** **Δ** and totals for **`mistimed-frame-count`**, **`frame-drop-count`**, **`decoder-frame-drop-count`**, which **signal** drives the budget (**mistimed** → **VO** → **decoder**), wall interval, and **~%** vs the same **denominator Hz** as overload (**mistimed** / **VO**: **≥ ~60 Hz**; **decoder**: **`container-fps`×`speed`** or **`estimated-vf-fps`**). **`stats`** **`eprintln`** is **skipped** once overload has shrunk ME on this **open media** **and** the strict-window strain rate is **below** the **overload** firing band (**~20%**); the stats bookkeeping still advances so the next visible line is not a long catch-up. **`smooth_budget`** strain ladders (decision logs + persistence) run **≈once per transport tick** only while the shell window is **visible**, **mapped**, **active**, and not **`GdkToplevel::MINIMIZED`** (**`smooth_budget_transport_window_ticks_count`**); busy-system **pause expiry** still runs when unfocused (**`smooth_load_hold_on_tick`**). **Besides** that sampler, **`[rhino] smooth: decision …`** logs **`overload`**, **`raise`** only when ME widens, **`persist_skip`**, and anomalies—**`raise_skipped`** lines are **not** emitted; recovery arms silently when decode fits cap, overload lock, CPU gate, or no step. Also **`[rhino] smooth: decision load_hold …`** / **`load_hold_resume`**.
...
- **Toolbar badge (contract):** playing FPS only (`smooth_toolbar_fps_label` ← mpv **`estimated-vf-fps`**). Selected Smooth must **not** paint a target **60**. With **`vapoursynth`** in **`vf`**, ignore estimates that still look like demux broadcast rates (~24/25/30) and show **—** until the filter-output estimate settles; never fall back to **`container-fps`** in that state. Without vapoursynth, fall back to source×**`speed`**. Tooltip keeps source → 60 intent.
- **Linux** (`vo=libmpv` + GTK **`GLArea`**): Plain and Smooth-on both prefer **`video-sync=display-resample`**, **`interpolation=no`**, and **`report_swap`** when the atomic gate is on. **First Smooth-on while playing** (empty **`vf`**, strip flag clear): **`smooth_user_enable_playing_reset`** — hold **`pause`**, **`loadfile replace`** at the playhead (no **`stop`**; it can abort the load), then debounced **`apply_mpv_video`** after **`FileLoaded`** / resume, then **`maybe_unpause_after_smooth_reload`**. **Reattach after strip** (**`smooth_vf_stripped_this_open`**): skip reload → **`smooth_reattach_after_vf_strip`**. **Post-seek / load-hold:** **`vf add`** only (no pause, no second seek, no **`loadfile`**). **User Smooth on** after a strip: **`arm_smooth_reattach_av_resync`** → pause-if-playing, **`vf add`**, **`absolute+exact`** to the pre-attach playhead, unpause (same A/V idea as audio-track resync) so a bare live attach cannot leave lip-sync drifted. On **`vf add`** failure, **`add_smooth_60`** reloads once (`smooth_vf_reload_attempted`) before clearing the pref. **`add_smooth_60`** skips attach while resume is pending; debounced resync retries **`apply_pending_resume`** then attaches. **`vf add` once per attach** (no retry loop / `vf set` — see VSScript pin). **First Smooth-on while paused**: live **`add_smooth_60`**. **Graph replace** (cadence rebuild with live **`vapoursynth`**): **`defer_smooth_vf_swap`** (`smooth-swap`). **Smooth off**: **`vf remove vapoursynth`** via **`clear_vf`**, then **`absolute+exact`** via **`smooth_off_refresh_playhead`** (same playhead realign as audio-track / toggle reattach — not keyframes). Do **not** use **`vf clr`** for teardown — mpv cannot **`vf add vapoursynth`** again on the same file after it. **`VF_SWAP_DEFER_IN_FLIGHT`** blocks stacked defers. **`schedule_smooth_60_resync_idle`** waits for **`resume_seek_pending`**. **320 ms** file-loaded hook updates speed env only when the graph already matches prefs. (No Smooth-off **`loadfile`** — sibling-folder EOF false positives.)
...
- MVTools plugin resolution order: `RHINO_MVTOOLS_LIB` env → search. **macOS** search: `.app` **`Contents/Resources/lib/vapoursynth/plugins/mvtools.dylib`**, then **`~/.config/rhino/lib/…`**, then Homebrew (and **seed** config via **`scripts/macos-vendor-smooth-libs.sh`** — **`mvtools.dylib`** + **`libfftw3f.3.dylib`**, **`@loader_path`**, ad-hoc codesign). macOS **does not** prefer a sticky SQLite `video_mvtools_lib` Cellar path before that search (it would skip seeding after `brew upgrade`). **Linux**: cached path if still a file, then distro / pipx / vsrepo search. Release **`.app`** packaging vendors into **`Contents/Resources/lib/vapoursynth`**. On success the path is saved to `video_mvtools_lib` and printed to stderr.
- macOS binding: stable plugin basename **`mvtools.dylib`** (Linux: `libmvtools.so`); owner **`paths_mvtools_macos.rs`**. VapourSynth + **libmpv** still come from Homebrew (VSScript / Python). mpv loads **`libvapoursynth-script.dylib`**; Homebrew R76 ships **`libvsscript.dylib`** — Rhino **re-execs** once at startup with **`DYLD_LIBRARY_PATH`** (**`paths_vapoursynth_macos.rs`**) plus a legacy-name symlink under **`~/.config/rhino/dylib`**. Before that re-exec, **`macos_ensure_vapoursynth_python_config`** repairs **`~/.config/vapoursynth/vapoursynth.toml`** only when the VSScript→Python mapping is **missing or its paths no longer exist** (e.g. dead Cellar pin after `brew upgrade python`). Live Cellar mappings (including VSScript’s optional third mtime field) are left alone so Rhino does not rewrite the file every launch. When repairing, Rhino writes stable **`opt/python@*`**. The bundled `.vpy` also prefers config / `.app` vendor paths before Homebrew at `LoadPlugin` time.

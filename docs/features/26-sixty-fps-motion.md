...
- The vf line is `vapoursynth:file=…:buffered-frames=16:concurrent-frames=auto` (fixed queue depth). From **`min(width,height)`**: **≤1440** (**hd** tier): **`blksize=16`**, **`overlap=8`**, **`pel=2`**, **`sharp=2`**, **`search=4`**, **`truemotion=True`**, **`chroma`** **on** (luma + chroma for **Super** and **Analyse**), **`FlowFPS`** **`mask=2`**, **`blend=True`**, **`Super`** uses MVTools default **`levels`** (**0** = automatic pyramid depth; explicit small values truncate the hierarchy and can gut blending); **>1440** (**uhd** tier): **`blksize=256`**, **`overlap=128`**, **`pel=1`**, **`sharp=1`**, **`search=4`**, **`FlowFPS`** **`mask=2`**, **`truemotion=True`**, **`blend=True`**, **`chroma`** **on** (luma + chroma for **Super** and **Analyse**). Stderr logs **`tier=`** plus parameters.

...
- mpv defaults are kept (`video-timing-offset` ≈ 0.05). **`mpv_render_context_report_swap`** on **Linux** is gated (**`SeqCst`**): **off** for plain **`video-sync=audio`**, **on** while Smooth vf drives **`video-sync=display-resample`**. **`vf clr`** keeps reporting swaps until **`restore_non_smooth_present_opts`** switches **`video-sync`** to **`audio`**, **then** clears the gate — mpv must not sit on **`display-resample`** without swaps. **macOS** plain playback keeps **`display-resample`** + **`report_swap`** (**`CVDisplayLink`** vsync); Smooth **off** previously tried **`audio`** and paced poorly against the fixed vsync thread.

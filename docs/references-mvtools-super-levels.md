...
Rhino’s bundled script runs **`mv.Super`** / **`Analyse`** / **`mv.FlowFPS`** on **native decode** when **`decode width × height ≤ RHINO_SMOOTH_MAX_AREA`** (persisted **`video_smooth_max_area`**, default ~**1920×1080**), else **`resize.Bicubic`** proportional downscale so ME pixels ≤ that budget (**stderr `path=scaled`**); output **stays** at the MVTools raster (**no** upscale). Stderr **`tier=uhd`** vs **`tier=hd`** remains **decode-area logging** (**≥2560×1440** vs smaller). **`Super`** / **`Analyse`** use **`chroma=true`** on **both** paths (luma-only ME breaks chroma). Same **`blksize` / `overlap`** (**`128` / `32`**); **`Super.levels`** automatic (**`0`** in the MVTools API — pyramid depth from frame size). **`Analyse`** uses **`search=4`** (hex), **`truemotion=true`**, **`global=true`** (**`Super`**: **`pel=1`**, **`sharp=1`**).

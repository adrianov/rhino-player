...
Rhino’s bundled script **downscales** decode for **ME** when **`decode width × height > RHINO_SMOOTH_MAX_AREA`** (persisted **`video_smooth_max_area`**, default ~**1920×1080**), runs **`mv.Super`** / **`Analyse`** / **`mv.FlowFPS`** on that proxy raster, and **`set_output()`** keeps **FlowFPS** dimensions — **vf** frame size may be smaller than decode (**stderr `vf_out=`**); **mpv** scales to the window (~**60** fps temporal smoothing). Stderr **`tier=uhd`** vs **`tier=hd`** remains **decode-area logging** (**≥2560×1440** vs smaller). **`Super`** / **`Analyse`** use **`chroma=true`** on **both** paths (luma-only ME breaks chroma). ME **width** / **height** are floored to multiples of **`blksize`** (``8``) before **`Super`** — **Crop** right/bottom on **full** decode when possible, else **Bicubic**; **scaled** path resizes to aligned size. Same **`blksize` / `overlap`** as MVTools upstream defaults (**`8` / `0`**); **`Super.levels`** automatic (**`0`** in the MVTools API — pyramid depth from frame size). **`Analyse`** uses **`search=4`** (hex), **`truemotion=true`**, **`global=true`** (**`Super`**: **`pel=1`**, **`sharp=1`**).

...
|---------------|-----------------|------|
| [vs/](vs/README.md) (`rhino_60_mvtools.vpy` + install notes) | (not installed) | Bundled **VapourSynth** + **MVTools** when **Preferences** → **Smooth Video (60 FPS)** is on, **speed ~1.0×**, and no custom `.vpy` is set |
...
| `mime/packages/ch.rhino.RhinoPlayer.xml` | `share/mime/packages` | shared-mime-info (in-progress Direct Connect `*.dctmp` → `application/x-dcpp-incomplete`) |

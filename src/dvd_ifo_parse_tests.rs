...
/// Sample VOB path comes from untracked local configuration: set `RHINO_DVD_17M_VOB` to the
/// `17_Mgnoveniy_DVD1` `VTS_02_1.VOB` when running sample tests locally. Skips silently
/// otherwise.
#[test]
fn chapter_marks_from_mounted_dvd_sample() {
    let Some(vob) = std::env::var_os("RHINO_DVD_17M_VOB").map(PathBuf::from) else {
        return;
    };
    if !vob.is_file() {
        return;
    }
    let main = main_title_from_disc(vob.parent().unwrap().parent().unwrap())
        .expect("VIDEO_TS.IFO main title");
...
/// Sample disc root comes from untracked local configuration: set `RHINO_DVD_FRITT_DIR` to a
/// Fritt Vilt (2006) DVD9 rip root when running sample tests locally. Skips silently otherwise.
#[test]

...
fn ntsc_film_prefers_estimated_when_container_rounds_to_24() {
    ...
    assert!(
        (source_fps_from_container_and_estimated(Some(24.0), Some(ntsc)).unwrap() - ntsc).abs()
            < 1e-6
    );
    assert!(
        (source_fps_from_container_and_estimated(Some(24.0), Some(24.0)).unwrap() - 24.0).abs()
            < 1e-6
    );
}
...
fn container_only_passthrough_including_24() {
    assert!(
        (source_fps_from_container_and_estimated(Some(24.0), None).unwrap() - 24.0).abs() < 1e-6
    );
}
...
fn interleaved_jump_keeps_display_resample_mode() {
    ...
    update_interleaved_cadence_gate(path.as_deref(), None, Some(30000.0 / 1001.0), &mut g);
    assert!(g.interleaved_smooth);
}
...
fn disc_stabilizer_ignores_wild_est_after_plausible_container() {
    ...
    assert_eq!(
        stabilize_disc_source_fps(path.as_deref(), None, Some(6.5), &mut g),
        first
    );
}
...
fn local_cadence_jump_keeps_mvtools() {
    let mut g = FpsPickGateState::default();
    for _ in 0..CADENCE_STABLE_READS {
        update_interleaved_cadence_gate(None, None, Some(24000.0 / 1001.0), &mut g);
    }
    assert!(!g.interleaved_smooth);
    update_interleaved_cadence_gate(None, None, Some(30000.0 / 1001.0), &mut g);
    assert!(!g.interleaved_smooth);
}
...
fn fps_gate_drops_stale_ntsc_after_opening_true_24_file() {
    ...
    for _ in 0..super::FPS_EST_IGNORE_READS_AFTER_PATH_CHANGE {
        assert_eq!(
            mask_est_for_path_change_with_state(
                Some("/holmes.mkv".into()),
                Some(ntsc),
                &mut g,
                None
            ),

...
fn try_load(
    ...
    let path = log_resolved_open_path(path);
    let tag = if o.warm_preload {
        "warm_preload"
    } else {
        "try_load"
    };
...
/// Resolve DVD folder indirection to the real media path and log the mapping.
fn log_resolved_open_path(path: &Path) -> PathBuf {
...
/// Preflight user-visible opens (toast callback present) and arm playback focus for play-on-start.
/// Warm preload skips both: failures there must stay silent and leave history untouched.
fn gate_non_warm_open(o: &LoadOpts, path: &Path) -> Result<(), String> {
    if !o.warm_preload {
...
/// Post-`loadfile` settle: window title, event drain, grid reveal, deferred drains / resync / hook.
fn settle_loaded_ui(
    player: &Rc<RefCell<Option<MpvBundle>>>,
    win: &adw::ApplicationWindow,
    gl: &gtk::GLArea,

...
//! `mv.Analyse` / `mv.FlowFPS`** tunables live in the bundled `.vpy`: **`tier=uhd`** when **`video_in`**
//! decode area is **≥2560×1440** (half-res MVTools + half-res output — see `data/vs/rhino_60_mvtools.vpy`), else **`tier=hd`** (native decode).

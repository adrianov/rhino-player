...
//! for **five successive** ticks with strict-window strain **>** **~40%** shrinks the saved ME budget by ~**10%** per firing (**`budget_after_decoder_overload`** — symmetric integer half-up step to recovery raise); **30 successive** ticks (~**30 s**) with relaxed-window strain **\<** **~20%** step **up** ~**10%** toward the **recovery ceiling** (decoded width×height when known, else **`DEFAULT_SMOOTH_MAX_AREA`**)

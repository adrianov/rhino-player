...
//! (first idle), when the user picks **playback speed** in the header (deferred idle), and on
...
//! After [loadfile] with Smooth on at ~1.0×, the first idle runs [apply_mpv_video] when playback is active.
...
//!
//! [try_load] schedules [apply_mpv_video] on the first idle after load. Other hooks (speed, Preferences)
//! call [apply_mpv_video] directly. Transport `pause=no` re-attaches via the same path when Smooth applies.

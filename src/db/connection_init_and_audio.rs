...
pub fn init() {
    ...
    migrate_dvd_thumb_chain_head_recapture(&conn);
    migrate_dvd_thumb_chain_seek_recapture(&conn);
...
fn migrate_dvd_thumb_chain_head_recapture(conn: &Connection) {
    let done: Option<String> = conn
        .query_row(
            "SELECT v FROM settings WHERE k = ?1",
            params![K_DVD_THUMB_CHAIN_HEAD_RECAPTURE_V1],
            |row| row.get(0),
        )
        .optional()
        .unwrap_or(None);
    ...
    let _ = conn.execute(
        "INSERT INTO settings (k, v) VALUES (?1, ?2)
         ON CONFLICT(k) DO UPDATE SET v = excluded.v",
        params![K_DVD_THUMB_CHAIN_HEAD_RECAPTURE_V1, "1"],
    );
}

fn migrate_dvd_thumb_chain_seek_recapture(conn: &Connection) {
    let done: Option<String> = conn
        .query_row(
            "SELECT v FROM settings WHERE k = ?1",
            params![K_DVD_THUMB_CHAIN_SEEK_RECAPTURE_V1],
            |row| row.get(0),
        )
        .optional()
        .unwrap_or(None);
    ...
    let _ = conn.execute(
        "INSERT INTO settings (k, v) VALUES (?1, ?2)
         ON CONFLICT(k) DO UPDATE SET v = excluded.v",
        params![K_DVD_THUMB_CHAIN_SEEK_RECAPTURE_V1, "1"],
    );
}
